...
#include "simpleunicodetournamenttrie.h"
...
SimpleUnicodeTournamentTrie::SimpleUnicodeTournamentTrie()
{
}
...
QString SimpleUnicodeTournamentTrie::GetName()
{
	return "Simple Unicode Tournament Trie";
}

int SimpleUnicodeTournamentTrie::GetFileFormatVersion()
{
	return 1;
}

SimpleUnicodeTournamentTrie::Type SimpleUnicodeTournamentTrie::GetType()
{
	return AddressLookup;
}

bool SimpleUnicodeTournamentTrie::LoadSettings( QSettings* /*settings*/ )
{
	return true;
}

bool SimpleUnicodeTournamentTrie::SaveSettings( QSettings* /*settings*/ )
{
	return true;
}

bool SimpleUnicodeTournamentTrie::Preprocess( IImporter* importer, QString dir )
{
	...
	std::vector< sutt::Node > subTrie( 1 );
...
void SimpleUnicodeTournamentTrie::insert( std::vector< sutt::Node >* trie, unsigned importance, const QString& name, sutt::Data data, sutt::PlaceData placeData )
{
	unsigned node = 0;
	...
	while ( position < lowerName.length() ) {
		...
		for ( int c = 0; c < ( int ) trie->at( node ).labelList.size(); c++ ) {
			sutt::Label& label = (*trie)[node].labelList[c];
			if ( label.string[0] == lowerName[position] ) {
				...
				if ( diffPos != label.string.length() ) {
					...
					trie->push_back( sutt::Node() ); //invalidates label reference!!!
...
void SimpleUnicodeTournamentTrie::writeTrie( std::vector< sutt::Node >* trie, QFile& file )
{
	if ( trie->size() == 0 )
		return;
...
#ifndef NOGUI
bool SimpleUnicodeTournamentTrie::GetSettingsWindow( QWidget** window )
{
	*window =  new UTTSettingsDialog();
	return true;
}

bool SimpleUnicodeTournamentTrie::FillSettingsWindow( QWidget* /*window*/ )
{
	return true;
}

bool SimpleUnicodeTournamentTrie::ReadSettingsWindow( QWidget* /*window*/ )
{
	return true;
}
